...
dataDir = '/Users/david/sila/marchmadness/2018/'
...
with open(dataDir+'NCAATourneyCompactResults.csv') as resultsFile:
...
with open(dataDir+'NCAATourneySeeds.csv') as seedsFile:
...
def scaleDay(daynum):
    return math.sqrt(daynum)

def calculateStatistics(rsry):
    for teamId, team in rsry.items():
        daySum = 0
        for game in team['games']:
            dayMultiplier = scaleDay(game['daynum'])
            daySum = daySum+dayMultiplier
            ...
            team['rawRatingSumDayWeight'] = team.get('rawRatingSumDayWeight',0.0)+game['rawRating']*dayMultiplier
            team['pythRatingSumDayWeight'] = team.get('pythRatingSumDayWeight',0.0)+game['pythRating']*dayMultiplier
    ...
    for teamId, team in rsry.items():
        ...
        for game in team['games']:
            ...
            team['oppRawRatingSumDayWeight'] = team.get('oppRawRatingSumDayWeight',0.0)+oppRawRating*dayMultiplier
            team['oppPythRatingSumDayWeight'] = team.get('oppPythRatingSumDayWeight',0.0)+oppPythRating*dayMultiplier
            ...
            if count > 0:
                ...
                uniqueOppOfOppDaySum += dayMultiplier
                team['oppAgainstNonOppSum'] = team.get('oppAgainstNonOppSum',0.0)+oneOppSum/count
                team['oppAgainstNonOppSumDayWeight'] = team.get('oppAgainstNonOppSumDayWeight',0.0)+(oneOppSum/count)*dayMultiplier
    ...
    for teamId, team in rsry.items():
        for game in team['games']:
            ...
            team['oppOppRawRatingSumDayWeight'] = team.get('oppOppRawRatingSumDayWeight',0.0)+oppOppRawRating*dayMultiplier
            team['oppOppPythRatingSumDayWeight'] = team.get('oppOppPythRatingSumDayWeight',0.0)+oppOppPythRating*dayMultiplier
            team['oppOppAgainstNonOppSum'] = team.get('oppOppAgainstNonOppSum',0.0)+rsry[game['opp']]['oppAgainstNonOppAvg']
            team['oppOppAgainstNonOppSumDayWeight'] = team.get('oppOppAgainstNonOppSumDayWeight',0.0)+rsry[game['opp']]['oppAgainstNonOppAvg']*dayMultiplier
